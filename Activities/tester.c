 ...
 *----------------------------------------------------------------------------------*/	
   ...
   // Problem #2 a)
   ProdList List = populateProdList();
   
   // Problem #2 b)
   displayProdList(List);
...
void displayProdList(ProdList L)
{
	...
	for(trav = L; trav != NULL; trav = trav->plink){
		...
		printf("\n\n%-7s", trav->prod.prodID);         //Complete this to display the ID of a choco record
		printf("%-12s", trav->prod.prodDesc.name);            //Complete this to display the name of a choco record
		printf("%-15d", trav->prod.prodDesc.weight );            // Complete this to dispaly the weight of a choco record
        count++;
    }
...
void deleteElem(ProdList *L, char ID[])
{
   ...
   for(trav = L; trav != NULL && strcmp((*trav)->prod.prodID, ID) != 0; trav = &(*trav)->plink){};
